<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage2.master" AutoEventWireup="true" CodeFile="state.aspx.cs" Inherits="state" %>

<asp:Content ID="Content1" ContentPlaceHolderID="holder1" Runat="Server">
    &nbsp;<table style="width: 100%">
        <form id="form1" runat="server">
        <tr>
            <td style="width: 347px">&nbsp;</td>
            <td style="width: 118px">
                <asp:Label ID="Label3" runat="server" Text="State operations"></asp:Label>
            </td>
            <td style="width: 143px">&nbsp;</td>
            <td style="width: 146px" colspan="2">&nbsp;</td>
            <td style="width: 45px">&nbsp;</td>
        </tr>
        <tr>
            <td style="height: 82px; width: 347px;"></td>
            <td style="width: 118px; height: 82px">
                <asp:Label ID="lblMsg" runat="server" BackColor="Red"></asp:Label>
            </td>
            <td style="width: 143px; height: 82px">
                <asp:Label ID="Label1" runat="server" Text="State Name:"></asp:Label>
            </td>
            <td style="width: 146px; height: 82px" colspan="2">
                <asp:DropDownList ID="drpS" runat="server" DataSourceID="SqlDataSource1" DataTextField="statename" DataValueField="statename">
                </asp:DropDownList>
                <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:tfinderConnectionString %>" SelectCommand="SELECT [statename] FROM [state]"></asp:SqlDataSource>
            </td>
            <td style="height: 82px; width: 45px;"></td>
        </tr>
        <tr>
            <td style="height: 25px; width: 347px;"></td>
            <td style="width: 118px; height: 25px"></td>
            <td style="width: 143px; height: 25px">
                <asp:Label ID="Label4" runat="server" Text="New Name:"></asp:Label>
            </td>
            <td style="width: 146px; height: 25px" colspan="2">
                <asp:TextBox ID="txtS" runat="server" Height="20px" Width="156px"></asp:TextBox>
            </td>
            <td style="height: 25px; width: 45px;"></td>
        </tr>
        <tr>
            <td style="height: 25px; width: 347px;">&nbsp;</td>
            <td style="width: 118px; height: 25px">&nbsp;</td>
            <td style="width: 143px; height: 25px">
                &nbsp;</td>
            <td style="width: 86px">
                <asp:Button ID="update" runat="server" OnClick="Button1_Click" Text="Update" Height="27px" />
            </td>
            <td style="height: 25px" colspan="2">
                <asp:Button ID="delete" runat="server" OnClick="Unnamed1_Click" Text="Delete" />
            </td>
            <td style="height: 25px">&nbsp;</td>
        </tr>
        <tr>
            <td style="height: 36px; width: 347px;">&nbsp;</td>
            <td style="width: 118px; height: 36px;">&nbsp;</td>
            <td style="height: 36px;" colspan="3">
                Enter New state Below,If state is not Available</td>
            <td style="height: 36px; width: 45px;">&nbsp;</td>
        </tr>
        <tr>
            <td style="height: 36px; width: 347px;"></td>
            <td style="width: 118px; height: 36px;"></td>
            <td style="width: 143px; height: 36px;">
                <asp:Label ID="Label2" runat="server" Text="New State:"></asp:Label>
            </td>
            <td style="width: 146px; height: 36px;" colspan="2">
                <asp:TextBox ID="txtS1" runat="server" Height="20px" Width="156px"></asp:TextBox>
            </td>
            <td style="height: 36px; width: 45px;"></td>
        </tr>
        <tr>
            <td style="width: 347px">&nbsp;</td>
            <td style="width: 118px">&nbsp;</td>
            <td style="width: 143px">&nbsp;</td>
            <td style="width: 86px">
                &nbsp;</td>
            <td style="width: 146px">
                <asp:Button ID="add" runat="server" OnClick="Button2_Click" Text="Add" />
            </td>
            <td style="width: 45px">&nbsp;</td>
        </tr>
        <tr>
            <td style="width: 347px">&nbsp;</td>
            <td style="width: 118px">&nbsp;</td>
            <td style="width: 143px">
                &nbsp;</td>
            <td style="width: 146px" colspan="2">&nbsp;</td>
            <td style="width: 45px">&nbsp;</td>
        </tr>
        <tr>
            <td style="width: 347px">&nbsp;</td>
            <td style="width: 118px">&nbsp;</td>
            <td style="width: 143px">&nbsp;</td>
            <td style="width: 146px" colspan="2">&nbsp;</td>
            <td style="width: 45px">&nbsp;</td>
        </tr>
    </table>
    </form>
</asp:Content>

